# 안동과외 (andong-tutoring) — 작업 규칙

경상북도 안동시 지역 타겟 과외 사이트. 정적 사이트, 빌드 없음.
...
## 도메인 / 배포
- 도메인: **nadoschool.co.kr** (가비아, 2026-09-21 연결). `CNAME` 파일 있음 · 주소 `https://nadoschool.co.kr/`
...
- 검색엔진 소유확인 메타는 `index.html` `<head>` 에 손으로 넣었다 (2026-09-22): 네이버 `56cc76cf1afea2ae37639f2b93c6c66657f13b2a`, 구글 `i5hi9LYCn60BrNUsoNNjlnrmYXXJxdRWpS3yCmkfsaM`. **생성기를 다시 돌리면 덮어써지니** 재생성 후에는 두 줄을 다시 넣거나, 생성기(`regions.js`)에 확인코드 항목을 추가할 것.

## 이 사이트만의 구조
| 파일 | 내용 |
|---|---|
| `index.html` | 홈 (히어로 SVG = undefined 모티프) |
| `schools.html` | 안동 초·중·고 56곳 목록 + 검색 |
| `school/*.html` | **학교별 정적 페이지 56개** — SEO 의 핵심. 학교당 국어·영어·수학·사회·과학·코딩 키워드 |
| `teachers.html` | 선생님 (화상 673명 — 안동 방문 선생님이 0명이라 방문 관련 내용은 감춰져 있다) |
| `online.html` | 화상과외 안내 |
...
5. `& "$env:USERPROFILE\사이트관리\로그기록.ps1" -Repo andong-tutoring -Status 완료 -What "글 N편"`
